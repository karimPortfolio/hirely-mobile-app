...
export function useResumeParsing(
  form: UseFormReturn<CreateApplicationPayload>,
) {
  ...
  const handleResumeSelect = useCallback(
    async (file: DocumentPickerAsset | null) => {
      ...
      try {
        const parsed = await parseResume(formData, abortController.signal).then(
          (res) => res.data ?? res,
        );
        ...
        const email =
          parsed?.email ||
          parsed?.contactInformation?.email ||
          parsed?.contact_information?.email;
        if (email) {
